...
void Camera::SetPositionSpherical(const glm::vec3& position_spherical) {
        m_position_spherical = position_spherical;
        UpdatePositionCartesian();

        // important this happens after updating the cartesian position, since it uses this as reference
        setOrthonormalBases();
        m_viewDirty = true;
        m_viewProjectionDirty = true;
}
...
void Camera::UpdatePositionCartesian() {
        m_position = Utils::sphericalToCartesian(m_position_spherical);
}
